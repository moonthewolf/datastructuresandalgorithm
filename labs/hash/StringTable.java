...
public class StringTable {
    ...
    {  
    	...
    	if (buckets[index] == null) {
    		...
    		return letsinsert;
    			...
    			ListIterator<Record> itr = buckets[index].listIterator();
    			while(itr.hasNext()) {
    				Record a = itr.next();
    ...
    public void remove(String key) 
    {
    	// TODO - implement this method
    	int hashcode = stringToHashCode(key);
    	int ibucket = toIndex(hashcode);
    	if (buckets[ibucket] == null) {
    		return;
    	}
    	else {
    			ListIterator<Record> itr = buckets[ibucket].listIterator();
    			while(itr.hasNext()) {
    				Record a = itr.next();
    				if (a.key.equals(key)) {
    					itr.remove();
    ...
    private int toIndex(int hashcode)
    {
    	...
    	double dindex = ((0.6153 * abshashcode) % 1.0 * nBuckets);
